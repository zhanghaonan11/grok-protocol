...
分步手动命令（注册 → 转换 JSON → 上传 CPA）见 **[手动执行.md](手动执行.md)**。
...
| `cpa_api_url` / `cpa_api_key` | CPA 接口地址与管理密钥 |
| `cpa_auto_upload` | 写出 JSON 时是否自动推 CPA（存量不会自动同步） |
| `cpa_use_local_name` | 推送时是否沿用本地文件名 |
| `cpa_skip_duplicates` | 远端已有同名/重复凭证时是否跳过 |
| `grok2api_*` | 可选写入 grok2api 本地/远端池 |

CPA 手动推送、SSO 补转 JSON 的完整命令见 **[手动执行.md](手动执行.md)**。
...
也可用 `sso_to_auth_json.py` 对账号行做鉴权换票，并把存量 JSON 推到 CPA；见 **[手动执行.md](手动执行.md)**。
...
| `sso_to_auth_json.py` | SSO / token → `xai_credentials`（鉴权 / 不鉴权） |
| `cpa_push.py` | 本地 JSON 推送到 CPA（自动 / 手动） |
...
| `手动执行.md` | 注册 / 转换 JSON / 上传 CPA 的手动命令说明 |
